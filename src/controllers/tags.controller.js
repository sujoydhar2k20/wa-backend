...
async function addToChat(req, res, next) {
    try {
        ...
        if (!chat.tags.includes(tag._id)) {
            if (tag.name && tag.name.toLowerCase() === 'monthly' && !['superadmin', 'admin', 'staff'].includes(req.user.role)) {
                return res.status(403).json({ success: false, message: 'Only superadmins, admins, and staff can assign the monthly tag.' });
            }
...
async function removeFromChat(req, res, next) {
    try {
        ...
        if (tag.name && tag.name.toLowerCase() === 'monthly' && !['superadmin', 'admin', 'staff'].includes(req.user.role)) {
            return res.status(403).json({ success: false, message: 'Only superadmins, admins, and staff can remove the monthly tag.' });
        }
